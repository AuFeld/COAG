...
from app.common.utils import async_timing, get_now
from app.database.utils import get_db_client, get_next_id
...
class MongoDBModel(BaseModel):
    ...
    .. code-block:: python
        ...
        mymodel = MyModel(addtional_field1="value")
        mymodel.save()
    ...
    def set_id(cls, v, values, **kwargs) -> int:
        '''
        If id is supplied (ex: from DB) then use it, otherwise generate now.
        '''
        if v: 
            return v
...
class MongoDBTimeStampedModel(MongoDBModel):
    ...
    .. code-block:: python 
        ...
        assert instance(mymodel.id, int)
        assert instance(mymodel.created, datetime)
    '''
    ...
    def set_created_now(cls, v: datetime) -> datetime: 
        '''
        If created is supplied (ex: from DB) -> use it, otherwise generate now.
        '''
        if v: 
            return v
        return get_now()