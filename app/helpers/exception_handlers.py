...
from pydantic import EnumError, StrRegexError, parse
...
def parse_error(
    ...
    '''
    Parse single error objext (such as pydantic-based or fastapi-based) to dict
...
def raw_errors_to_fields(raw_errors: List) -> List[dict]:
    '''
    Translates list of raw errors (instances) into list of dicts w/ name/msg

    :param raw_errors: List with instances of raw error
    :return: List of ficts (1 dict for every raw error)
    '''
    fields = []
    for top_err in raw_errors:
        if hasattr(top_err.exc, "raw_errors"):
            for err in top_err.exc.raw_errors:
                ...
                # handling and internal validation 
...
async def http_exception_handler(
    ...
    Handles StarletteHTTPException, translating it into flat dict error data:
        ...
        * feilds - list of dicts with description of the error in each field
...
async def validation_exception_handler(
    request: Request, exc: RequestValidationError) -> UJSONResponse:
    """
